...
3. **E4 importer (in progress):** `importer/e4_import.py` parses an E4 Connect session (directory or `.zip` — ACC/BVP/EDA/HR/TEMP/IBI/tags) into an `E4Session` with absolute UTC timestamps for post-hoc alignment. Exposed via `POST /api/import/e4` and a web Import panel. Row-based parser core, fully unit-tested headless.
...
5. **Frontend build-out (in progress):** componentized dashboard — device cards with live signal-quality bars (consuming the SQI), modality panel, stream monitor, session bar (recording + elapsed), activity log, and a Kinect skeleton-preview canvas fed by a `joints` WS event (`DeviceManager.broadcast_joints` + `project_skeleton_sample`). `tsc` strict + `vite build` green.
